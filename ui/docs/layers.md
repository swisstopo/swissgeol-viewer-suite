...
- `Layer` is a type made up of all *normal* layer types, which are:
  - `WmtsLayer`
  - `TiffLayer`
  - `Tiles3dLayer`
  - `VoxelLayer`
- `AnyLayer` is a type made up of all *available* layer types,
...
const isLayerActive$: Observable<boolean> = layerService.isLayerActive$(layerId);
