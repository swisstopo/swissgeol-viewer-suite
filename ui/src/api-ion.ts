type AssetStatus = 'AWAITING_FILES' | 'NOT_STARTED' | 'IN_PROGRESS' | 'COMPLETE' | 'ERROR' | 'DATA_ERROR';
type AssetType = '3DTILES' | 'GLTF' | 'IMAGERY' | 'TERRAIN' | 'KML' | 'CZML' | 'GEOJSON';
...
export type GetAssetOptions = {
  ...
  sortOrder?: 'ASC' | 'DESC';
  sortBy?: 'ID' | 'NAME' | 'DESCRIPTION' | 'BYTES' | 'TYPE' | 'STATUS' | 'DATE_ADDED';
  search?: string;
  page?: number;
  limit?: number;
};
