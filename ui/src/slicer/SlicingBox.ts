...
export default class SlicingBox extends SlicingToolBase {
  ...
  globePlanes = [];
  planesPositions:
    | [[Cartesian3, Cartesian3], [Cartesian3, Cartesian3], [Cartesian3, Cartesian3], [Cartesian3, Cartesian3]]
    | null = null;
  ...
  activate(options: SlicingBoxOptions): void {
    ...
    this.slicingBoxEntity = this.dataSource.entities.add({
      ...
      polylineVolume: {
        ...
        shape: new CallbackProperty(() => {
          // Fixme workaround, see https://github.com/CesiumGS/cesium/issues/12005
          this.viewer.scene.requestRender();
          return [Cartesian2.ZERO, Cartesian2.ZERO, scratchCartesian2, new Cartesian2(0, this.bbox!.height)];
        }, false),
      },
    });
    ...
    if (typeof options.showBox === 'boolean') this.toggleBoxVisibility(options.showBox);
    this.viewer.scene.requestRender();
  }
